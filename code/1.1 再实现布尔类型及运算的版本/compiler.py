...
table = []  # table = [{'name': ID, 'type': , 'address': , 'size': }] 符号表
...
def p_declaration_stat(p):
    ...
    if len(p) == 4:
        ...
        enter(p[2], p[1][1], local_address, 1)
        local_address += 1
    else:
        p[0] = ('declaration_stat', p[1], p[2], p[4])
        enter(p[2], p[1][1], local_address, int(p[4]))
...
def enter(name, type, address, size):
    global tx
    if tx >= table_max_num:
...
if __name__ == '__main__':
    ...
    if len(error_list) == 0:
        ...
        for i in range(len(table)):
            ftable.write(str(i + 1) + '\t' + table[i]['name'] + '\t' + table[i]['type'] + '\taddress = '
                         + str(table[i]['address']) + '\tsize = ' + str(table[i]['size']) + '\n')
            print(str(i + 1), '\t', table[i]['name'], '\t', table[i]['type'], '\taddress =',
                  table[i]['address'], '\tsize =', table[i]['size'])
