...
table = []  # table = [{'name': ID, 'kind': ,'type': , 'value': , 'level': , 'address': , 'size': }] 符号表
...
def p_block(p):
    '''block : gen_jmp declaration_list rec_local_adr procedure_list gen_ini statement_list call_stat
             | gen_jmp declaration_list rec_local_adr procedure_list gen_ini statement_list'''
    if len(p) == 8:
        p[0] = ('block', p[1], p[2], p[3], p[4], p[5], p[6], p[7])
    else:
        p[0] = ('block', p[1], p[2], p[3], p[4], p[5], p[6])
    gen('opr', 0, 0)

def p_rec_local_adr(p):
    'rec_local_adr : '
    p[0] = ('rec_local_adr')
    local_address_list.append(local_address)
...
def p_procedure_list(p):
    '''procedure_list : procedure_list procedure_stat
                      ...
                      |'''
    if len(p) == 3:
        p[0] = ('procedure_list', p[1], p[2])
    elif len(p) == 2:
        p[0] = ('procedure_list', p[1])
    else:
        p[0] = ('procedure_list', '')

def p_procedure_stat(p):
    'procedure_stat : procedure_init LBRACE block RBRACE'
    global lev
    p[0] = ('procedure_stat', p[1], p[3])
    lev -= 1
    local_address_list.pop()

def p_procedure_init(p):
    'procedure_init : procedure ID'
    global lev, local_address
    if lev >= lev_max_num:
        print('Too many nesting levels!')
        error_list.append('Too many nesting levels!')
        return
    p[0] = ('procedure_init', p[2])
    enter(p[2], 'procedure', '', -1, lev, 1, 0)
    lev += 1
    local_address = 3
...
def p_gen_ini(p):
    'gen_ini : '
    p[0] = ('gen_ini')
    gen('ini', 0, local_address_list[-1])
    for example in reversed(table):
        if example['kind'] == 'procedure' and example['address'] == 1 and example['size'] == 0:
            example['address'] = cx - 1
            example['size'] = local_address_list[-1]
            break
...
def p_declaration_stat(p):
    '''declaration_stat : type ID SEMICOLON
                        ...
                        | const type ID EQUAL MINUS NUMBER SEMICOLON'''
    global local_address
    if len(p) == 4:
        p[0] = ('declaration_stat', p[1], p[2])
        enter(p[2], 'variable', p[1][1], -1, lev, local_address, 1)
        local_address += 1
    elif len(p) == 8:
        p[0] = ('declaration_stat', p[2], p[3], p[5], p[6])
        enter(p[3], 'constant', p[2][1], -int(p[6]), -1, -1, -1)
    else:
        if p[1] == 'const':
            p[0] = ('declaration_stat', p[2], p[3], p[5])
            enter(p[3], 'constant', p[2][1], int(p[5]), -1, -1, -1)
        else:
            p[0] = ('declaration_stat', p[1], p[2], p[4])
            enter(p[2], 'variable', p[1][1], -1, lev, local_address, int(p[4]))
...
def p_call_stat(p):
    ...
    for example in table:
        if example['name'] == p[2]:
            isDefined = 1
            if example['kind'] == 'procedure':
                isProcedure = 1
                lev_pro = example['level']
                cal_adr = example['address']
            break
    if isDefined == 0:
        print("ID '%s' undefined!" % p[1])
        error_list.append("ID '%s' undefined!" % p[1])
        return
    if isProcedure == 0:
        print("ID '%s' is not procedure!" % p[1])
        error_list.append("ID '%s' is not procedure!" % p[1])
    else:
        gen('cal', lev - lev_pro, cal_adr)
...
def p_read_stat(p):
    ...
    for example in table:
        if example['name'] == p[2][1]:
            if example['size'] == 1:
                # p[2] = ('var', ID)
                gen('sto', lev - example['level'], example['address'])
            else:
                # p[2] = ('var', ID, ('expression', ('simple_expr', ('logical_expr', ('additive_expr', (
                #         'term', ('self_operating', ('factor', NUM)))))))
                gen('sto', lev - example['level'], example['address'] + int(p[2][2][1][1][1][1][1][1][1]))
            break
                ...
                if example['name'] == p[2][1][1][1][1][1][1][1][1]:
                    if example['address'] == -1 and example['size'] == -1:
                        ...
                        for i in range(example['size']):
                            gen('lod', lev - example['level'], example['address'] + i)
                            gen('opr', 0, 14)
                    break
        else:
            # 输出表达式
...
def p_self_operating(p):
    ...
    if len(p) == 2:
        ...
        if p[1] == '++':
            p[0] = ('self_operating', 'operator_front', p[1], p[2])
            gen('opr', 0, 17)
        elif p[1] == '--':
            p[0] = ('self_operating', 'operator_front', p[1], p[2])
            gen('opr', 0, 18)
        elif p[2] == '++':
            p[0] = ('self_operating', 'operator_back', p[2], p[1])
            gen('opr', 0, 19)
        elif p[2] == '--':
            p[0] = ('self_operating', 'operator_back', p[2], p[1])
            gen('opr', 0, 20)
        elif p[1] == 'not':
            p[0] = ('self_operating', p[1], p[2])
            gen('opr', 0, 26)
                ...
                if example['name'] == p[1][1]:
                    if len(p[1]) == 2:
                        ...
                        if example['kind'] == 'variable':
                            gen('lod', lev - example['level'], example['address'])
...
def enter(name, kind, type, value, level, address, size):
    global tx
    if tx >= table_max_num:
        print('Too many identifiers!')
        error_list.append('Too many identifiers!')
        return
    new_ID = {}
    new_ID['name'] = name
    new_ID['kind'] = kind
...
def gen(f, l, a):
    ...
    if cx >= code_max_num:
        ...
        error_list.append('Program is too long!')
        return
    if a >= address_max:
        print('Displacement address is too big!') # 地址偏移越界
        error_list.append('Displacement address is too big!')
        return
    new_code = {}
    new_code['f'] = f
    new_code['l'] = l
...
def interpret():
    ...
    while p != cx:
        ...
        if current_code['f'] == 'ini':
            ...
            if current_code['a'] < 0:
                for example in table:
                    if example['address'] == -current_code['a']:
                        if s[t - 1] >= 0 and s[t - 1] < example['size']:
            ...
            if current_code['a'] == 0:
                ...
                for example in table:
                    if (example['address'] <= instruction[p]['a'] and
                        instruction[p]['a'] <= example['address'] + example['size'] - 1) \
                            or example['address'] == -instruction[p]['a']:
                        # 读入单个变量、索引为常数的数组、索引为表达式的数组
                        if example['type'] == 'int':
                            try:
                                tmp = int(argv_list[parameter])
                                if tmp >= -2147483648 and tmp <= 2147483647:
                                    t += 1
                                    s[t] = tmp
                                    fresult.write('Input?    ' + argv_list[parameter] + '\n')
                                    parameter += 1
                                else:
                                    print("The range of type 'int' is -2147483648 ~ 2147483647!")
                                    error_list.append("The range of type 'int' is -2147483648 ~ 2147483647!")
                                ...
                                error_list.append("The type of input should be 'int'!")
                        elif example['type'] == 'char':
                            try:
                                tmp = ord(argv_list[parameter])
                                t += 1
                                s[t] = tmp
                                fresult.write('Input?    ' + argv_list[parameter] + '\n')
                                parameter += 1
                                ...
                                error_list.append("The type of input should be 'char'!")
                        elif example['type'] == 'bool':
                            if argv_list[parameter] == 'true':
                                t += 1
                                s[t] = 1
                                fresult.write('Input?    ' + argv_list[parameter] + '\n')
                                parameter += 1
                            elif argv_list[parameter] == 'false':
                                t += 1
                                s[t] = 0
                                fresult.write('Input?    ' + argv_list[parameter] + '\n')
                                parameter += 1
                                ...
                                error_list.append("The type of input should be 'bool'!\nPlease input 'true' or 'false'!")
                        break
...
def x0_compiler(data, input_list):
    global cx, tx, lev, parameter, local_address, argv_list
    ...
    for_start_adr.clear()
    isbreak_list.clear()
    switch_start_adr.clear()

    error_before_interpret = 0
        ...
        for i in range(len(table)):
            if table[i]['kind'] == 'constant':
                ftable.write(str(i + 1) + '\t' + table[i]['name'] + '\t' + table[i]['kind'] + '\t' + table[i]['type']
                            + '\tvalue = ' + str(table[i]['value']) + '\tlevel =  \taddress =  \tsize =  \n')
                print(str(i + 1), '\t', table[i]['name'], '\t', table[i]['kind'], '\t', table[i]['type'],
                    '\tvalue =', table[i]['value'], '\tlevel =  \taddress =  \tsize =  ')
            else:
                ftable.write(str(i + 1) + '\t' + table[i]['name'] + '\t' + table[i]['kind'] + '\t' + table[i]['type']
                             + '\tvalue =  \tlevel = ' + str(table[i]['level']) + '\taddress = '
                             + str(table[i]['address']) + '\tsize = ' + str(table[i]['size']) + '\n')
                print(str(i + 1), '\t', table[i]['name'], '\t', table[i]['kind'], '\t', table[i]['type'],
                      '\tvalue =  \tlevel =', table[i]['level'], '\taddress =', table[i]['address'],
                      '\tsize =', table[i]['size'])
